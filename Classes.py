...
class JWST:
  ...
  def JWST_propagate(JWST_IO:ct.NonlinearIOSystem,
      ...
      response = ct.input_output_response(JWST_IO, timepts=timepts, inputs=inputs, initial_state=X0,
  ...
  def optimal_transfer_orbit(JWST_IO:ct.NonlinearIOSystem,X0,Xf,u0,uf,
    ...
    if cost is 'Quadratic':
      ...
      constraints = [opt.input_range_constraint(JWST_IO,-100/M/a_C*[1,1,1],100/M/a_C*[1,1,1])]
    else:
      print('Cost function is undefined.')
    
    result = opt.solve_optimal_trajectory(JWST_IO,
                                          timepts,
                                          X0,
                                          traj_cost,
                                          constraints,
...
class ThreeBodySystem:                     
  ...
  def Dimensionalize(self,outputs:Tuple,time):
    x = self.r_12 * outputs['x']
    y = self.r_12 * outputs['y']
    z = self.r_12 * outputs['z']
    xdot = self.V_C * outputs['xdot']
    ydot = self.V_C * outputs['ydot']
    zdot = self.V_C * outputs['zdot']
    t = self.t_C * time
